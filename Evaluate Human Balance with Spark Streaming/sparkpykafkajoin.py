...
zSetEntriesEncodedCustomerDF=spark.sql("select key, zSetEntries[0].element as encodedCustomer from RedisSortedSet")
...
zSetEntriesDecodedCustomerDF= zSetEntriesEncodedCustomerDF.withColumn("encodedCustomer", unbase64(zSetEntriesEncodedCustomerDF.encodedCustomer).cast("string"))

# TO-DO: parse the JSON in the Customer record and store in a temporary view called CustomerRecords
zSetEntriesDecodedCustomerDF\
    .withColumn("customerName", from_json("encodedCustomer", customerJSONSchema))\
