...
class Line:
    ...
    def process_message(self, message):
        ...
        if message.topic() == 'chicogo.consumers.faust.table': # Set the conditional correctly to the stations Faust Table
            try:
                value = json.loads(message.value())
                self._handle_station(value)
            except Exception as e:
                logger.fatal("bad station? %s, %s", value, e)
        elif message.topic() == 'chicago.stations': # Set the conditional to the arrival topic
